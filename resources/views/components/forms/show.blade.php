...
<div class="container mx-auto p-4">
    <div
        class="bg-white dark:bg-gray-700 p-8 rounded-xl shadow-2xl border border-gray-300 dark:border-gray-700 transition-all duration-300 ease-in-out">
        @if (isset($fields['title']))
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ $item->{$fields['title']} }}</h1>
        @endif

        <!-- Información del recurso -->
        <div class="grid grid-cols-1 {{ count($fields['attributes']) > 1 ? 'sm:grid-cols-2' : '' }} gap-4">
